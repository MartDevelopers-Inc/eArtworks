...
if (isset($_POST['Add_To_WishList'])) {
    $wishlist_product_id = mysqli_real_escape_string($mysqli, $_POST['wishlist_product_id']);
    $wishlist_user_id = mysqli_real_escape_string($mysqli, $_POST['wishlist_user_id']);

    /* Prevent Duplications */
    $sql = "SELECT * FROM wishlists  WHERE wishlist_product_id ='{$wishlist_product_id}' AND  wishlist_user_id = '{$wishlist_user_id}'";
    $res = mysqli_query($mysqli, $sql);
    if (mysqli_num_rows($res) > 0) {
        $row = mysqli_fetch_assoc($res);
        if ($wishlist_user_id == $row['wishlist_user_id'] && $wishlist_product_id == $row['wishlist_product_id']) {
            $err = "Product already in your wishlist";
        }
    } else {
        /* Persist */
